                                ...
                                @foreach($datas as $data)
                                    ...
                                    <tr>
                                        <td>
                                            <div class="icheck-primary">
                                                <input type="checkbox" value=""
                                                       class="forms-check-input" data-id="{{$data->comment_id}}" id="XXcheck1">
                                                ...
                                                @if(auth()->user()->can('authenticated-user-can-access'))
                                                    <a href="{{ route('post.show',['post'=> $data->post_id])}}">
                                                        ...
                                                        <p class="text-muted mb-0">
                                                            {{ Illuminate\Support\Str::limit($data->post->title,30, '') }}
                                                            <b>view Comment</b>
                                                        </p>
                                                    </a>
                                                @endif
                                        ...
                                        <td>
                                            @auth
                                                @if(auth()->user()->can('authenticated-user-can-access'))
                                                    {{ Illuminate\Support\Str::limit($data->body,30, '') }}
                                                    <a href="{{ route('dashboard.user.comment.edit',['comment'=> $data->comment_id])}}" class="" data-url=""
                                                       data-id="">
                                                        edit
                                                    </a>
                                                @endif
                                                @if(auth()->user()->can('admin-can-access'))
                                                        {{ Illuminate\Support\Str::limit($data->body,30, '') }}
                                                        <a href="{{ route('post.show',['post'=> $data->post_id])}}" class="" data-url=""
                                                           data-id="">
                                                            Show
                                                        </a>
                                                @endif
                                        ...
                                        <td>
                                            @auth
                                                @if(auth()->user()->can('authenticated-user-can-access'))
                                                    {!! Form::open(['method' => 'DELETE', 'route' => ['dashboard.user.comment.destroy', $data->comment_id],
                                                    'onsubmit' => 'return confirm("Are you sure?")', 'id'=>'himan']) !!}
                                                    <button><i class="far fa-trash-alt"></i></button>
                                                    {!! Form::close() !!}
                                                @endif
                                                @if(auth()->user()->can('admin-can-access'))
                                                        {!! Form::open(['method' => 'DELETE', 'route' => ['dashboard.comment.destroy', $data->comment_id],
                                                        'onsubmit' => 'return confirm("Are you sure?")', 'id'=>'himan']) !!}
                                                        <button><i class="far fa-trash-alt"></i></button>
                                                        {!! Form::close() !!}
                                                @endif
